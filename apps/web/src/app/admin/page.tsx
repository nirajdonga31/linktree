...
function SortableLinkItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`bg-white p-4 rounded-2xl border transition-all duration-200 mb-3 group hover:shadow-md ${
        link.isActive ? 'border-gray-200' : 'border-gray-100 opacity-70'
      }`}
    >
          ...
          >
            <div
              className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${
                link.isActive ? 'translate-x-5.5' : 'translate-x-0.5'
              }`}
              style={{ transform: link.isActive ? 'translateX(20px)' : 'translateX(2px)' }}
            />
          </button>
...
export default function AdminPage() {
  ...
  return (
    <div>
      ...
      {/* Links List */}
      <div className="space-y-1 max-h-[calc(100vh-500px)] overflow-y-auto pr-2">
        {loading ? (
